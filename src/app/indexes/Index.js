...
const ensureValueValidForField = (value, field) => {
  ...
  if (!value && field.type === "Collection") {
    throw new Error(
      `document does not have a value for field ${field.name}. Collection fields must have a value`,
    );
  }
  if (field.type === "Int64" && value && Number.isNaN(parseInt(value, 10))) {
    throw new Error(
      `document has value ${value} for Int64 field ${field.name}, which is not a valid Int64`,
    );
  }
  if (field.type === "DateTimeOffset" && value && !(value instanceof Date)) {
    throw new Error(
      `document has value ${value} for DateTimeOffset field ${field.name}, which is not a valid date`,
    );
  }
};
...
const attemptToStoreDocuments = async (
  name,
  keyFieldName,
  documents,
  attempts = 3,
  ...
  const failedDocuments = responseDocuments.filter(
  ...
  if (errorDocuments.length > 1) {
    const statusCodes = [
      ...new Set(errorDocuments.map((doc) => doc.statusCode)),
    ].join();
    logger.error(
      `Documents failed to index into "${name}", status code(s): ${statusCodes}`,
      {
  ...
  if (retryKeys.length > 1) {
    if (attempts === 1) {
      const statusCodes = [
        ...new Set(responseRetryDocuments.map((doc) => doc.statusCode)),
      ].join();
      logger.error(
        `Documents failed to index into "${name}" after multiple retries, status code(s): ${statusCodes}`,
        {
    ...
    const retryDocuments = documents.filter((doc) =>
      retryKeys.includes(doc[keyFieldName]),
    );
...
class Index {
  ...
  async store(documents, correlationId) {
    ...
    await forEachAsync(batches, async (batch, index) => {
      logger.debug(
        `Writing batch ${index + 1} of ${batches.length} to ${this.name}`,
        { correlationId },
      );
      try {
        const keyField = Object.entries(this.structure).find(
        ...
        throw new Error(
          `Error writing batch ${index} to ${this.name} - ${e.message}`,
        );
      }
    });
  }
    ...
    try {
      ...
      if (filters) {
        ...
        filters.forEach((filter) => {
          ...
          if (!filter.values) {
            throw new Error(
              `All filters must have a values (Missing on ${filter.field})`,
            );
          }
          const field = this.structure[filter.field];
          if (!field) {
            throw new Error(
              `Field ${filter.field} is not in the index structure for ${this.name}`,
            );
          }
      ...
      throw new Error(
        `Error searching ${this.name} using criteria '${criteria}' (page=${page}, pageSize=${pageSize}, sortBy=${sortBy}, sortAsc=${sortAsc}, filters=${JSON.stringify(filters)}) - ${e.message}`,
      );
    }
  }
  ...
  async delete(id) {
    try {
      ...
      throw new Error(
        `Error deleting document with id ${id} from index ${this.name} - ${e.message}`,
      );
    }
  }
}
