...
describe("Search Index store function", () => {
  beforeEach(() => {
    ...
    preparedDocuments = rawDocuments.map((doc) => ({
      ...doc,
  ...
  it("Will throw an error if no field in the structure is specified as the key", async () => {
    ...
    await expect(index.store(rawDocuments, "testing")).rejects.toThrow(
      `No field set as the key for Index ${indexName}`,
    );
  });
  ...
  it("Will attempt to store the documents if a field is specified as the key", async () => {
    ...
    expect(storeDocumentsInIndex).toHaveBeenCalledWith(
      indexName,
      preparedDocuments,
    );
  });
  ...
  it("Will log the response documents, if the response code is 207 and none of them can be retried", async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(
      `Documents failed to index into "${indexName}", status code(s): ${errorStatus}`,
      {
  ...
  it("Will attempt to retry storing documents, if the response code is 207 and all of them can be retried (3 attempts default)", async () => {
    ...
    expect(storeDocumentsInIndex.mock.calls[0][1].length).toStrictEqual(
      rawDocuments.length,
    );
    expect(storeDocumentsInIndex.mock.calls[1][1].length).toStrictEqual(
      rawDocuments.length,
    );
    expect(storeDocumentsInIndex.mock.calls[2][1].length).toStrictEqual(
      rawDocuments.length,
    );
  });
  ...
  it("Will attempt to retry storing documents that can be retried, if the response code is 207 and some of them fail on retries", async () => {
    ...
    const secondResponseDocuments = firstResponseDocuments.map(
      (doc, index) => ({
        key: doc.key,
        status: false,
    ...
    const thirdResponseDocuments = secondResponseDocuments.map(
      (doc, index) => ({
        key: doc.key,
        status: false,
  ...
  it("Will log any response documents that need to be retried, if they are not indexed after the attempt limit (3 attempts default)", async () => {
    ...
    expect(logger.error).toHaveBeenLastCalledWith(
      `Documents failed to index into "${indexName}" after multiple retries, status code(s): ${retryStatus}`,
      {
        documents: thirdResponseDocuments.filter(
          (doc) => doc.statusCode !== 400,
        ),
      },
    );
  });
